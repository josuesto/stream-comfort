...
Earlier live controls, manual navigation, and the user's confirmation that popup stuttering stopped are retained in the [0.3.3 report](VERIFICATION-0.3.3.md) and its linked history. The [manual checklist](../MANUAL-TESTS.md) identifies remaining installed checks, particularly natural endings and fullscreen. Reload the extension and refresh streaming pages once after updating; ordinary preference changes then apply without reloads.
